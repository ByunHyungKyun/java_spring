...
public class ApplicationContextInitListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent ctxEvent)  { 
		...
		try {
			...
			Document document = documentBuilder.parse(beanConfigXml);//객체화
			...
			for(int i = 0 ; i < beans.getLength();i++){//baen인스턴스 생성단계
				...
				if(bean.getNodeType() == Node.ELEMENT_NODE) {
					...
					Object targetObj = cls.newInstance();//싱글톤 적용,bean마다 인스턴스생성
					...
					for(int j = 0 ; j < properties.getLength();j++){
						...
						if(property.getNodeType() == Node.ELEMENT_NODE) {
							...
							for(Method method : methods) {
								//의존성 여부 확인
								System.out.println("확인"+method.getName());
